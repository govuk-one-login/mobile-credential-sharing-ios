...
struct SessionDecryptionTests {
    ...
    private func setUpTestData() throws -> (
        ...
        let privateKey = try P256.KeyAgreement.PrivateKey(rawRepresentation: Data(staticPrivateKey))
        return (privateKey: privateKey, key: eReaderPublicKey)
    }
    ...
    mutating func decryptDataThrowsSessionDecryptionError() async throws {
        let testData = try setUpTestData()
    ...
    mutating func decryptDataThrowsDecryptionError() async throws {
        let testData = try setUpTestData()
        #expect(messageCounter == 1)
