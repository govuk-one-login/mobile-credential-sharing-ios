...
@Suite("CertificateValidityPeriod Contract Tests")
struct CertificateValidityPeriodContractTests {
    @Test("Construction and property access")
    func construction() {
        let now = Date()
        let later = Date(timeIntervalSinceNow: 86400)

        let validity = CertificateValidityPeriod(notBefore: now, notAfter: later)

        #expect(validity.notBefore == now)
        #expect(validity.notAfter == later)
    }
    ...
    func equatable() {
        ...
        let validity = CertificateValidityPeriod(notBefore: now, notAfter: later)
        let same = CertificateValidityPeriod(notBefore: now, notAfter: later)
        let different = CertificateValidityPeriod(notBefore: later, notAfter: later)

        #expect(validity == same)
        #expect(validity != different)
    }

    @Test("Conforms to Sendable")
    func sendableConformance() {
        let _: any Sendable = CertificateValidityPeriod(notBefore: Date(), notAfter: Date())
    }
}
...
struct CoseVerificationResultContractTests {
    ...
    func attachedConstruction() {
        ...
        let validity = CertificateValidityPeriod(
            notBefore: Date(),
            notAfter: Date(timeIntervalSinceNow: 86400)
        )
        ...
        #expect(result.certificateValidity == validity)
    }
    ...
    func sendableConformance() {
        let _: any Sendable = CoseVerificationResult(
            ...
            payload: nil,
            certificateValidity: CertificateValidityPeriod(notBefore: Date(), notAfter: Date())
        )
    }
}
