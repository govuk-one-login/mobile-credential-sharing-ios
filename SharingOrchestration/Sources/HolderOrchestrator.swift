...
public class HolderOrchestrator: HolderOrchestratorProtocol {
    ...
    func performPreflightChecks() {
        ...
        guard let prerequisiteGate = prerequisiteGate else {
            delegate?.orchestrator(self, didUpdateState: .error("PrerequisiteGate is not available."))
            return
        }
        do {
                ...
                if permissionsToRequest.contains(where: { $0 == .bluetooth(.bluetoothStateUnknown) }) {
                    ...
                    for permission in permissionsToRequest {
                        ...
                        case .bluetooth(.bluetoothAuthDenied), .bluetooth(.bluetoothAuthRestricted):
                            delegate?.orchestrator(self, didUpdateState: .error(permission.rawValue))
                        default:
                            delegate?.orchestrator(self, didUpdateState: session?.currentState)
                        }
                    }
                }
            }
        } catch {
            delegate?.orchestrator(self, didUpdateState: .error(error.localizedDescription))
        }
    ...
    func prepareEngagement() {
        ...
        guard let session = session else {
            delegate?.orchestrator(self, didUpdateState: .error("Session is not available."))
            return
        }
    ...
    private func presentQRCode() {
        guard let qrCode = session?.qrCode else {
            delegate?.orchestrator(self, didUpdateState: .error("QR Code failed to generate."))
            return
        }
...
extension HolderOrchestrator: BluetoothTransportDelegate {
    ...
    public func bluetoothTransportDidFail(with error: PeripheralError) {
        delegate?.orchestrator(self, didUpdateState: .error(error.errorDescription ?? "Unknown error"))
    }
